        ...
        # convert input video to required format
            ...
            try:
                ...
                raise P1203StandaloneError(
                    'Could not auto-generate input report, error: {e.output}'.format(e=e)
                )
        else:
            raise P1203StandaloneError(
...
def main(modules={}, quiet=False):
    ...
    parser.add_argument(
        '--print-intermediate', action='store_true', help='print intermediate O.21/O.22 values'
    )
    parser.add_argument(
        '--cpu-count', type=int, default=multiprocessing.cpu_count(), help='thread/CPU count'
    )
    parser.add_argument('--version', action='version', version=str(__version__))
    parser.add_argument(
        '--accept-notice', action='store_true', help='accept license and acknowledgement terms'
    )
    parser.add_argument(
