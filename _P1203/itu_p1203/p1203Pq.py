...
class P1203Pq(object):
    ...
    def _calc_stalling_features(self, duration):
        ...
        total_stall_len = sum(
            [
                l_buff
                * utils.exponential(
                    1, self.coeffs['c_ref7'], 0, self.coeffs['c_ref8'], duration - p_buff
                )
    ...
    def calculate(self):
        ...
        if self.amendment_1_stalling:
            q_fac = min(
                max(self.coeffs['amd_1_a1'] * total_stall_len + self.coeffs['amd_1_a2'], 0), 1
            )
            O46 = 1 + (O46 - 1) * q_fac
    ...
    def _calc_034_035_baseline(self, duration):
        ...
        for t in range(duration):
            ...
            if self.amendment_1_audiovisual:
                # Eq. 17a
                O34[t] = (1 - max(0, self.coeffs['amd_1_a_threshold'] - self.O21[t])) * (
                    O34[t] - 1
                ) + 1
    ...
    def _calc_and_test_osc(
        ...
        if osc_test:
            ...
            osc_comp = np.maximum(
                ...
                np.minimum(
                    q_diff
                    * np.exp(self.coeffs['comp1'] * q_dir_changes_tot + self.coeffs['comp2']),
                    1.5,
                ),
            )
        return osc_comp
