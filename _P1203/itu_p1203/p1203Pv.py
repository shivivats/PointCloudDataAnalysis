...
class P1203Pv(object):
    _COEFFS = {
        ...
        'q2': -0.07,
        'q3': 4.06,
        'mode0': {'a1': 11.9983519, 'a2': -2.99991847, 'a3': 41.2475074001, 'a4': 0.13183165961},
        'mode1': {
            'a1': 5.00011566,
            'a2': -1.19630824,
            'a3': 41.3585049,
    ...
    def degradation_due_to_frame_rate_reduction(self, deg_cod_v, deg_scal_v, framerate):
        ...
        if framerate < 24:
            deg_frame_rate_v = (
                (100 - deg_cod_v - deg_scal_v) * (t1 - t2 * framerate) / (t3 + framerate)
            )
    ...
    def video_model_function_mode0(
        ...
        quant = a1 + a2 * np.log(
            ...
            + np.log(
                bitrate_kbps_segment_size * bitrate_kbps_segment_size / (coding_res * framerate)
                + a4
            )
        )
    ...
    def video_model_function_mode2(
        self, coding_res, display_res, framerate, frames, quant=None, avg_qp_per_noni_frame=[]
    ):
        """
        Mode 2 model
                ...
                for frame in frames:
                    ...
                    if frame_type not in ['I', 'P', 'B', 'Non-I']:
                        raise P1203StandaloneError(
                            'frame type ' + str(frame_type) + ' not valid; must be I/P/B or I/Non-I'
                        )
            ...
            if any('representation' in f for f in frames):
                ...
                score = self.video_model_function_mode0(
                    ...
                    utils.resolution_to_number(
                        frames[output_sample_index].get('displaySize') or self.display_res
                    ),
