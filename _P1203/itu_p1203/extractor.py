...
class Extractor(object):
    ...
    def extract(self):
        ...
        for segment in self.input_files:
            ...
            (segment_info_video, segment_info_audio, duration) = Extractor.get_segment_info_lines(
                segment,
                mode=self.mode,
        ...
        report = {
            'IGen': {'displaySize': '1920x1080', 'device': 'pc', 'viewingDistance': '150cm'},
    ...
    def _parse_qp_data(logfile, use_average=False):
        ...
        for line in Extractor._file_line_gen(logfile):
            ...
            if ('[h264' in line or '[mpeg2video' in line) and 'pkt_size' not in line:
                ...
                line_qp_values = [
                    int(raw_values[i : i + 2].lstrip()) for i in range(0, len(raw_values), 2)
                ]
    ...
    def get_video_frame_info_ffmpeg_debug_qp(segment, qp_logfile=None, use_average=False):
        ...
        ffmpeg_debug_script = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', 'ffmpeg-debug-qp', 'ffmpeg_debug_qp')
        )
        ...
        if mode == 1:
            ...
            for frame in frame_info:
                frame_stats_json.append(
                    {'frameType': frame['frame_type'], 'frameSize': frame['size']}
                )
...
def main(_):
    ...
    parser.add_argument(
        '-q', '--qp-logfile', type=str, help='existing logfile generated by ffmpeg_debug_qp'
    )
    parser.add_argument(
        '-a', '--use-average', action='store_true', help='use average QP (saving memory/space)'
    )
